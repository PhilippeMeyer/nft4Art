...
async function mintSpheres() {
    ...
    fs.readdirSync(dirFolder).forEach(element => {
        ...
        if (collection[colstr] === undefined) { // Initializing the collection with a set to store all the tokens
            collection[colstr] = {};
            collection[colstr].tokenSet = new Set();
        }
        if (element.substring(2, 6) == '_img')
            collection[colstr].image = element;
        else if (element.substring(2, 6) == '_map')
            collection[colstr].map = element;
        else {
