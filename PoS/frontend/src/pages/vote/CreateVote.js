...
export default function CreateVote() {
    ...
    const [header, setHeader] = useState({ title: '', start: new Date(), end: new Date() });
    ...
    function save() {
        ...
        .then((response) =>  { 
            if(response.status == 200) enqueueSnackbar('Vote transfered to the server');
            else enqueueSnackbar('Error in transfer vote to server status: %s', response.statusText); 
        })
        .catch((error) => { 
